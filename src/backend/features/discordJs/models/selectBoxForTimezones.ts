...
class DateMenuInput {
    ...
    static toMenuItemsFromArr(data: MenuItemData[]) {
        const items = DateMenuInput.fromArr(data);
        return items.map((data) => data.toMenuItem());
    }

    static fromArr(data: MenuItemData[]) {
        return data.map((data) => new DateMenuInput(data));
    }
}

const timezonesPositivesArr: MenuItemData[] = [
    {
        label: "UTC+0",
...
const timezonesPositivesOptions = DateMenuInput.toMenuItemsFromArr(
    timezonesPositivesArr
);
const timezonesNegativesOptions = DateMenuInput.toMenuItemsFromArr(
    timezonesNegativesArr
);
...
export const timezonesPositivesCopy = new StringSelectMenuBuilder()
    .setCustomId("positives")
    .setPlaceholder("Positive timezones UTC+")
    .setMaxValues(1)
    .setMinValues(1)
    .addOptions(...timezonesPositivesOptions);
