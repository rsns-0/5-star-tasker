...
export class DeepLService {
	...
	public async validateThenTranslate({ text, targetLanguage, textValidationSchema }: ValidateTranslateTextArgs) {
	...
	public async sendTranslationDataToAPI({ text, targetLanguage }: TranslateTextArgs) {
		...
		try {
			const { data: res } = await this.client.post<DeepLDataCollection>(this.apiEndpoint, body);
			data = res;
		} catch (e) {
		...
		return res.data.translations.map((translationData) => new TranslationData(translationData, targetLanguage));
	}

	private static cleanText(textStrOrTextArr: string | string[], textValidationSchema = defaultValidTextSchema) {
