...
/**
 * Expected folder structure: testResources - testfile1.ts - testfile2.ts
 * testDir1 - testfile3.ts
 */
...
describe("getExportsFromFilesInFolder", () => {
	...
	it("should throw an error if the specified folder is not found", () => {
		expect(() =>
			descendToFolderThenGetExportsFromFolder("nonExistentFolder", startingPoint)
...
describe("goToAncestorFolder", () => {
	...
	it("should throw an error if the specified folder is not found", () => {
		expect(() =>
			descendToFolderThenGetExportsFromFolder(
				"nonexistentFolder238jf9283jwenf9wijwi10191238jdsf2nn3nhed88",
				__dirname
			)
		).toThrowError(TraverserError);
	});
});
