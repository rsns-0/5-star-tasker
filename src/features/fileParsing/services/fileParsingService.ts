...
/**
 * Starts from the start position and performs bfs to find the first folder with
 * the specified name. Afterwards, retrieves all exports from files within the
 * folder.
 *
 * @example
 ...
 *
 * @template TExportModel - The interface representing the export object from a
 *   module.
 * @param folderName - The name of the folder to search for files in.
 * @param startPosition - The starting position of the search. Due to
 *   limitations, cannot create a default. Pass in __dirname to start from the
 *   current directory.
 * @param options - An optional object containing the following properties:
 *
 *   - FileFilter: A function that determines whether a file should be included in
 *       the search. Defaults to .ts and .js files.
 *   - Recursive: A boolean indicating whether to search recursively through
 *       subfolders for file exports after reaching the target folder. Defaults
 *       to false.
 *
 * @returns A promise that resolves to an array of exported models.
 * @throws {TraverserError} - If the folder is not found or if the starting
...
/**
 * Starts from a given directory and ascends until it reaches the first folder
 ...
 * @param startingDirectory - The starting directory path to begin the traversal
 *   from.
