import { parseToCleanedStackframe as parseToCleanedStackFrame, stripDirectoryPath } from '../utils/stackUtils';

import { formatTestDescription } from './testUtils/descriptions';
import { type StackFrame, parse } from 'stacktrace-parser';
...
describe('parseToCleanedStackframe', () => {
	it.each(testDataArray)(formatTestDescription('should strip out unrelated directories correctly'), (str) => {
...
describe('TraceData', () => {
	...
	it('should parse information correctly for first item', () => {
		expect(first.file).toBe('C:\\RandomDir1\\AnotherDir\\5-star-tasker\\src\\tests\\prisma.test.ts');
	...
	it('should parse information correctly for last item which has a method name', () => {
		expect(resultWithMethodName.file).toBe('file:///F:/FinalDir/5-star-tasker/node_modules/@vitest/runner/dist/index.js');
